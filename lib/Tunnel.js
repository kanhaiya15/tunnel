...
function Tunnel() {
  ...
  this.start = function (options, fnCallback) {
    ...
    _verifyToken(options, function (e, res) {
      if (e) {
        ELKLogger(options['user'], options['key'], { filename: __filename },options, ('Getting error while verifying user and key . Error Response is : ' + (res || e)));
  ...
  this.stop = function (fnCallback) {
    try {
      ...
      _killRunningProcess(that, function (e) {
        if (e) {
          ELKLogger(that.options['user'], that.options['key'], { filename: __filename },that.options, ('Getting error while we are trying to kill the running process. Throws Error : ' + (e)));
          return fnCallback(e);
        }
        return fnCallback();
      });
    } catch (e) {
      ELKLogger(this.options['user'], this.options['key'], { filename: __filename },this.options, ('Something unexpected while trying to kill process. Error : ' + (e)));
      return fnCallback(e);
    }
  };
...
function _runBinary(self, retries, fnCallback) {
  if (retries >= 0) {
    _addArguments(self, function (e, binaryArguments) {
      if (e) {
        ELKLogger(self.options['user'], self.options['key'], { filename: __filename }, self.options, ('Getting this error while adding argument . Error : ' + (e)));
      ...
      self.proc.stderr.pipe(split()).on('data', function (data) {
        ELKLogger(self.options['user'], self.options['key'], { filename: __filename },self.options, ('We are getting this error while we are trying to spawning process. Error :  ' + (data)));
      ...
      self.proc.on('exit', function (code) {
        ...
        ELKLogger(self.options['user'], self.options['key'], { filename: __filename }, self.options, ('Getting this message while tunnel is being closed. Exit code is : ' + (code)));
        self.isProcessRunning = false;
        if (!isCallback) {
          return fnCallback(null, false);
        }
    ...
    ELKLogger(self.options['user'], self.options['key'], { filename: __filename }, self.options, 'Number of retries to run binary exceeded.');
    return fnCallback(null, false);
  }
}
...
function _verifyToken(options, fnCallback) {
  try {
    request.post({
      ...
      if (e) {
        ELKLogger(options['user'], options['key'], { filename: __filename }, options, ('We are getting this error while verifying user and key validity. Error : ' + (e)));
        ...
        if (json && json.type === 'error') {
          ELKLogger(options['user'], options['key'], { filename: __filename }, options, ('Getting this issue while user is not valid' + (json)));
    ...
    ELKLogger(options['user'], options['key'], { filename: __filename }, options, ('Something unexpected while verifying the user and key validity. Error : ' + (e)));
    return fnCallback(true, e);
  }
};
...
function _killRunningProcess(that, fnCallback) {
  try {
    ...
    ELKLogger(that.options['user'], that.options['key'], { filename: __filename }, that.options,('Getting error while killing the running process. Error : ' + (e)));
    return fnCallback(e);
  }
};
...
function _addArguments(self, fnCallback) {
  try {
    ...
    _getFreePort(options, 5, function (e, port) {
      ...
      ELKLogger(options['user'], options['key'], { filename: __filename }, options, ('Info of passed tunnel arguments ' + (binaryArgs)));
    ...
    getPort(function (e, port) {
      if (e) {
        if (retries >= 0) {
          ...
          ELKLogger(options['user'], options['key'], { filename: __filename }, options, ('Error trying to get Free Port on LambdaTest Tunnel' + e));
  ...
  try {
    ...
    if (retries >= 0) {
      ...
      request.get(url, function (e, response, json) {
        if (e) {
          ELKLogger(self.options['user'], self.options['key'], { filename: __filename }, self.options, ('Getting error while trying to get the tunnel name from running local server. Error : ' + (e)));
      ...
      ELKLogger(self.options['user'], self.options['key'], { filename: __filename }, self.options, 'Number of retries to to get tunnel name exceeded.');
      return fnCallback(null);
    }
  } catch (e) {
    ELKLogger(self.options['user'], self.options['key'], { filename: __filename }, self.options, ('Something unexpected while trying to get tunnel name from local server. Error : ' + (e)));
    return fnCallback(null);
  }
}
