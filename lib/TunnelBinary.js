...
function TunnelBinary() {
  ...
  if (this.hostOS.match(/darwin|mac os/i)) {
    ...
    this.hostOS.match(/mswin|msys|mingw|cygwin|bccwin|wince|emc|win32/i)
  ) {
    this.windows = true;
    this.httpPath =
  ...
  this._retryBinaryDownload = function(
    conf,
    destParentDir,
    fnCallback,
    retries,
  ...
  this._download = function(conf, destParentDir, fnCallback, retries) {
    try {
      ...
      console.error("Got Error while downloading binary zip");
      ELKLogger(conf["user"], conf["key"], { filename: __filename }, conf, {
  ...
  this._binaryPath = function(conf, fnCallback) {
    try {
      ...
      console.log(e);
      ELKLogger(conf["user"], conf["key"], { filename: __filename }, conf, e);
    }
  };
  ...
  this._fetchHash = function(conf, retries, fnCallback) {
    try {
      ...
      if (retries >= 0) {
        ...
        https
          .get(httpHashPath, function(response) {
